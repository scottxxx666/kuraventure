...
Status: milestone 1 (scaffold) done — game boots to a placeholder menu. Next: PLAN.md
milestone 2 (world core + input).
...
- `yarn dev` serves on port 8080 with `host: true` so a phone on the same LAN can
  ...
  `yarn typecheck`. **The user runs all scripts manually — never run them yourself.**
